# @taktikorg/dolor-deleniti
## (localStorage on the server)

### Super-easy asynchronous persistent data structures in Node.js, modeled after HTML5 localStorage

Node-persist doesn't use a database. Instead, JSON documents are stored in the file system for persistence. Because there is no network overhead, @taktikorg/dolor-deleniti is just about as fast as a database can get. Node-persist uses the HTML5 localStorage API, so it's easy to learn.
...
* If you're looking for the version that supports both `synchronous` and `asynchronous` use `@taktikorg/dolor-deleniti@2.1.0`

## Install

```sh
$ npm install @taktikorg/dolor-deleniti
```

## Basic Example

```js
const storage = require('@taktikorg/dolor-deleniti');
...
* Added the `writeQueue*` options, trying to resolve [issue#108](https://github.com/taktikorg/dolor-deleniti/issues/108), see the API Documentation below.
...
* `storage.key()` is gone
* The default `dir` is now `process.cwd() + (dir || '.@taktikorg/dolor-deleniti/storage')`, unless you use an absolute path
...
##### Options
You can pass `init()` an options object to customize the behavior of @taktikorg/dolor-deleniti
